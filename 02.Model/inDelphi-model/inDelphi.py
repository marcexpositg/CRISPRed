...
## Custom function to add "-" in gaps
def add_genotype_column_wgaps(pred_df, stats):
  new_pred_df = pred_df
  gts = []
  if type(stats) == dict:
    seq = stats['Reference sequence']
  ...
  for idx, row in new_pred_df.iterrows():
    ...
    if gt_pos == 'e':
      ...
      dl = row['Length']
      gt_pos = int(gt_pos)
      #modify to add '-' in deletion site and position
      gt = seq[:cutsite - dl + gt_pos] + '-' * dl + seq[cutsite + gt_pos:]
    else:
      ins_base = row['Inserted Bases']
      gt = seq[:cutsite] + ins_base + seq[cutsite:]
    gts.append(gt)
